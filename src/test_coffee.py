...
if selection is not None:
    ...
    for i, motion in enumerate(motions):
        ...
        max_p = Point(2,2,1)
        min_p = Point(-1,-1,0.5)
        res = add_grid('base_footprint', 'grid' + str(i), 0.03, 0.05, min_p, max_p, pcl.points)

    position = cluster[2].point
    position.x -= (0.12 + cluster[2].size.x/2.0) # gripper is 12cm, and half of the object
    selection = '/base_footprint', cluster[1], position
